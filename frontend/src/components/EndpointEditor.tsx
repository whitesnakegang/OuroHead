...
interface EndpointEditorProps {
  ...
  onPreview: (snapshot: Endpoint) => void;
}
 ...
 *
 * @param {EndpointEditorProps} props
 * @param {Endpoint} props.endpoint - Initial endpoint configuration to edit (method, path, description, requiresAuth, authType, authHeader, responses, request, etc.).
 * @param {(updated: Endpoint) => void} props.onUpdate - Callback invoked whenever the endpoint configuration changes; receives the updated endpoint object.
 * @param {(snapshot: Endpoint) => void} props.onPreview - Callback invoked when the user requests a preview; receives the current local endpoint snapshot.
...
function EndpointEditor({
  ...
  const [localEndpoint, setLocalEndpoint] = useState<Endpoint>(endpoint);

  useEffect(() => {
    setLocalEndpoint(endpoint);
  }, [endpoint]);

  const handleChange = (field: keyof Endpoint, value: any): void => {
    const updated = { ...localEndpoint, [field]: value };
    setLocalEndpoint(updated);
    onUpdate(updated);
  };

  const handleResponsesChange = (responses: any[]): void => {
    const updated = { ...localEndpoint, responses };
    setLocalEndpoint(updated);
    onUpdate(updated);
  };

  const addStatusResponse = (): void => {
  ...
  const updateStatusResponse = (index: number, updates: any): void => {
    const newResponses = [...(localEndpoint.responses || [])];
    newResponses[index] = { ...newResponses[index], ...updates };
    handleResponsesChange(newResponses);
  };

  const deleteStatusResponse = (index: number): void => {
    const newResponses = (localEndpoint.responses || []).filter(
      (_, i) => i !== index
    );
    handleResponsesChange(newResponses);
  };
  ...
  return (
    <div className="endpoint-editor">
      ...
      <div className="editor-content">
                        ...
                        onChange={(fields) => {
                          updateStatusResponse(index, {
                            response: {
                              ...statusResp.response,
                              arrayItemType: {
                                ...statusResp.response.arrayItemType,
                                fields,
                              },
                            },
                          });
        ...
        {(localEndpoint.request?.type === "body" ||
          localEndpoint.request?.type === "query") && (
          <div className="form-group">

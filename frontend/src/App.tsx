...
function App(): JSX.Element {
  ...
  const loadApiDefinition = async (): Promise<void> => {
    try {
      setLoading(true);
  ...
  const updateEndpoint = (index: number, updatedEndpoint: Endpoint): void => {
  ...
  const deleteEndpoint = (index: number): void => {
    if (window.confirm("Are you sure you want to delete this endpoint?")) {
      const newEndpoints = apiDefinition.endpoints.filter(
        (_, i) => i !== index
      );
      setApiDefinition({ ...apiDefinition, endpoints: newEndpoints });
